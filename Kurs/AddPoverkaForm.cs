...
namespace Kurs
{
    public partial class AddPoverkaForm : Form
    {
        private SqlDataAdapter _dataAdapter = new SqlDataAdapter();

        public AddPoverkaForm()
        {
            InitializeComponent();
        }

        private void AddPoverkaForm_Load(object sender, EventArgs e)
        {
            res_.SelectedIndex = 0;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                connection.Open();
                using (var command = new SqlCommand("P16", connection))
                {
                    try
                    {
                        ...
                        command.Parameters.Add(new SqlParameter("@ФиоПоверяющего", fio_.Text));
